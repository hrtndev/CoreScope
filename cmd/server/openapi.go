...
func routeDescriptions() map[string]routeMeta {
	return map[string]routeMeta{
		...
		"GET /api/scope-coverage": {Summary: "Hash-region coverage areas", Description: "Groups repeaters/rooms by the MeshCore hash regions they've actually relayed traffic for (transported_scopes, derived from non-ADVERT packets carrying that region's scope) and returns a convex hull of each region's member node positions — an inferred coverage area, not an authoritative boundary (hash regions carry no shape of their own). A node's own self-broadcast scope (default_scope) is NOT used — only relay evidence. Excludes blacklisted/hidden/foreign-flagged nodes. Cached 30s.", Tag: "admin"},
