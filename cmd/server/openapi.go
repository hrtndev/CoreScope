...
func routeDescriptions() map[string]routeMeta {
	return map[string]routeMeta{
		...
		"GET /api/scope-coverage": {Summary: "Hash-region coverage areas", Description: "Groups nodes by their matched MeshCore hash region (nodes.default_scope) and returns a convex hull of each region's member node positions — an inferred coverage area, not an authoritative boundary (hash regions carry no shape of their own). Excludes blacklisted/hidden/foreign-flagged nodes. Cached 30s.", Tag: "admin"},
